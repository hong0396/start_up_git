...
titanic_all.loc[:, 'Prefix'].loc[:, titanic_all['Prefix'].isin(['Capt', 'Col', 'Major'])] = 'Officer'
...
titanic_all.loc[:, 'Prefix'].loc[:, titanic_all['Prefix'].isin(['Don', 'Sir', 'Jonkheer'])] = 'Sir'
...
def show_performance(model):
    ...
    probabilities = model.predict_proba(train_data[predictors])[:, 1]
    print('Performance Report')
    print('Accuracy : {:.2%}'.format(metrics.accuracy_score(train_data['Survived'].values, predictions)))
    print('AUC Score (Test): {0}'.format(metrics.roc_auc_score(train_data['Survived'], probabilities)))
