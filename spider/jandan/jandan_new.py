...
def get_urls(url):
    ...
    for tag in tags:
        ...
        print(img_url)
        img_large=str(img_url).replace('mw600','large')
        name=str(img_hash)
        name=name[:3]+name[-3:]
        name = name.replace(r'/','-')
        name = name.replace('\\','-')
        if not os.path.exists("E:\\picture\\" + name):
            print('---------正在写入jpg--------------')
            with open("E:\\picture\\" + name+".jpg", 'wb') as file:
                textj = requests.get('http:' + img_large, stream=True)
...
if __name__ == '__main__':
    ...
    get_urls('http://jandan.net/ooxx/page-80')
