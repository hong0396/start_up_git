...
for code_nm in li_code:
    print('------------------------------------------'+str(nu_nu)+'----------------------------------------------')
    url='https://xueqiu.com/stock/finance_us_income_statement.json?symbol='+str(code_nm)+'&data_type=1&dateAscType=desc&_='+ti    
    for nn in li_sum:
        nn.append('0')
    time.sleep(0.1)
        ...
        for i in li:
            ...
            for j in range(len(li_time)):
                if tmp_date == li_time[j]:
                    if not tmp_earn is None:
                        li_sum[j][nu_nu]=tmp_earn
    else:
        print('下载数据错误')
    nu_nu=nu_nu+1
...
re.to_csv(date+'_xueqiu_us_earn.csv', encoding = 'gbk',index=False)
