...
def get_laohu_analysis_all_rs(n, url, li_code,days,earn): 
    ...
    con = requests.get(url_dji, headers=header).json()  
    li_data=con.get('items')    
    if li_data is not None:
        dji_pd=pd.DataFrame(li_data)
        dji_pd['time']=dji_pd['time'].apply(todate)
...
days_df=get_grow_code(url_day,10, li_code)
...
for i in range((len(codee)//100)+1):
    ...
    get_laohu_analysis(i, url_day, code_tmp, days_tmp,earn_tmp)
    time.sleep(1)
    get_laohu_analysis_rs(i, url_day, code_tmp, days_tmp,earn_tmp)
    # time.sleep(1)
