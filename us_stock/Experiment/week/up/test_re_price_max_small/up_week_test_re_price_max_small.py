        ...
        if con.get('error') is None:               
            ...
            if li_data is not None:
                ...
                if len(jo.time.tolist()) > 60:
                    ...
                    for i in range(days):
                        if  zong.iloc[i]["close"] >= max(zong.close.tolist()):
...
def get_laohu_analysis(n, url, li_code,days,earn,pee): 
    ...
    for nmm in range(len(li_code)):
        ...
        if li_data is not None:
            ...
            if not '-' in str(earn[nmm]).split('_')[2]:
                ax.set_title(str(li_code[nmm])+'('+str(days[nmm])+')_'+str(int(bio*100))+'_'+str(pee[nmm]).split('.')[0]+'_'+str((str(earn[nmm]).split('_')[3].strip('%')).split('.')[0].replace(',', ''))+'%',fontsize=18,fontweight='bold')        
            else:
                ax.set_title(str(li_code[nmm])+'('+str(days[nmm])+')_'+str(int(bio*100))+'_'+str(pee[nmm]).split('.')[0]+'_'+str((str(earn[nmm]).split('_')[3].strip('%')).split('.')[0].replace(',', ''))+'%',fontsize=18,fontweight='bold',color="darkgreen")    
