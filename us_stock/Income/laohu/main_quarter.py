...
for code_nm in li_code:
    print('------------------------------------------'+str(nu_nuu)+'----------------------------------------------')
    url='https://hq.itiger.com/fundamental/usstock/earnings/income/'+code_nm+'?type=income&symbol='+code_nm+'&deviceId=web20180825_733304&platform=desktop-web&env=Chrome&vendor=web&lang=&appVer=4.1.0'
    time.sleep(0.1)
                    ...
                    if li[i].get('type') == '季报':
                        ...
                        for k in range(len(li_time)):
                            if year_date in li_time[k]:
                                li_sum[k][nu_nuu]=str(value)+'_'+yoy
...
re.to_csv(date+'_Laohu_us_profit_quarter.csv', encoding = 'gbk',index=False)
