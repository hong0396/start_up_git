...
code=pd.read_csv('D:\\Git\\us_stock\\ROE\\2018-08-05_all_us_basic.csv',encoding='gbk')                
...
def get_roe(code):
    global nu_nu
    print('--------------------------'+str(nu_nu)+'--------------------------------')
    url='https://hq1.itiger.com/fundamental/usstock/earnings/income/'+code+'?type=income&symbol='+code+'&deviceId=web20180805_726777&platform=desktop-web&env=Chrome&vendor=web&lang=&appVer=4.1.0'
    res=requests.get(url, headers=headers)
    a=res.json()
    li=a.get('data').get('page')
    ...
    url='https://hq1.itiger.com/fundamental/usstock/earnings/balance/'+code+'?type=balance&symbol='+code+'&deviceId=web20180805_726777&platform=desktop-web&env=Chrome&vendor=web&lang=&appVer=4.1.0'
    res=requests.get(url, headers=headers)
    a=res.json()
...
re.to_csv(date+'_Laohu_us_roe.csv', encoding = 'gbk',index=False)
