...
def get_grow_code(url, li_code): 
    ...
    for code_nm in li_code:
        ...
        if li_data is not None:
            ...
            sma_output = SMA(jo, timeperiod=40, price='close')
            x = jo['time'].values
            y = jo['close'].values
            x_li = jo['time'].tolist()
            y_li = jo['close'].tolist()
            ...
            #筛选40周线大于股票价格
            if sma_output.tolist()[-1] > y_li[-1]:
                #筛选股票正在处于上升势头
                if dder1_list[-1] > 0 and dder1_list[-2]  > 0:
                    localmin_tmp=getmin(p1,x_li[-1],x_li[-3])
                    new_x_li = [i for i in x_li if i > localmin_tmp]
                    #筛选股票上升时间大于一月
                    if len(new_x_li)> 4 : #
...
def get_laohu_analysis(n, url, li_code): 
    ...
    plt.savefig("fig_net_"+str(n)+".png")
    # plt.show()
    
            
        

codee=get_grow_code(url_week, li_code)
