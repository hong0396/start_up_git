...
code=pd.read_csv('D:\\Git\\us_stock\\gainings\\laohu\\gainings_picture\\pe0_grow.csv',encoding='gbk')
...
def get_laohu_analysis(n, url, li_code): 
    ...
    for code_nm in range(len(li_code)):
        ...
        con = requests.get(url.format(str(li_code[code_nm])), headers=header).json()
        time.sleep(0.1) 
        ...
        if li_data is not None:
            ...
            # gainings2017=code['2017_profit'][code_nm]
            # gainings2016=code['2016_profit'][code_nm]
            # gainings2015=code['2015_profit'][code_nm]
            # gainings2014=code['2014_profit'][code_nm]

            # li_xx=[2014,2015,2016,2017]
            # li_ttmp=[gainings2014,gainings2015,gainings2016,gainings2017]

            # # ax.subplot(211)
            # ax.bar(li_xx, li_ttmp)   
            # # ax.subplot(212)   
            ax.set_title(str(li_code[code_nm])+'('+str(year)+')',fontsize=18,fontweight='bold')    
            plot1=ax.plot(x, y, '.', markersize=15,label='original values')
            plot2=ax.plot(x, yvals, 'r',label='polyfit values') 
            # ax.legend(loc=4)
            # g=sns.relplot(x= 'time', y='close' , data=jo, s=75,  ax=axes[nu_nu//10, nu_nu%10 ])
            ax.annotate(' ', xy=(time_max, close_max), xytext=(time_max, close_max),arrowprops=dict(facecolor='red', shrink=0.05),)
