...
code=pd.read_csv('D:/Git/us_stock/laohu/basic_code/2018-09-20us_all_code.csv',encoding='gbk')
            ...
            if not li is None:
                ...
                for i in range(len(li)):
                    if li[i].get('type') == '年报':
                        ...
                        if num2 < len(tmmp):                     
                            valu_income=tmmp[num2].get('value')                            
                        ...
                        for k in range(len(li_time)):
                            if year_date in li_time[k]:
                                li_sum[k][nu_nu]=str(value)+'_'+str(value_income)
...
re.to_csv(date+'_Laohu_us_year_gainings.csv', encoding = 'gbk',index=False)
