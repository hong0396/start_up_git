    ...
    for code_nm in li_code:
        ...
        if li_data is not None:
            ...
            if len(jo.time.tolist()) > 30:
                ...
                for i in range(5):
                    if zong.iloc[i]['open'] >= zong.iloc[i+1]['open'] and zong.iloc[i+1]['open'] >= zong.iloc[i+2]['open']:
                    # if zong.iloc[1]['open'] > zong.iloc[2]['open']:
                        if zong.iloc[i+2]['open'] >= zong.iloc[i+3]['open'] and zong.iloc[i+3]['open'] >= zong.iloc[i+4]['open'] and zong.iloc[i+4]['open'] >= zong.iloc[i+5]['open']:      
...
def get_laohu_analysis(n, url, li_code): 
    ...
    plt.savefig("fig_6_days_up_price_"+str(n)+".png")
    # plt.show()
