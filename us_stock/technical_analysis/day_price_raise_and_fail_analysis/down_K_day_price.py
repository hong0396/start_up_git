...
def get_grow_code(url, li_code): 
    ...
    for code_nm in li_code:
        ...
        if li_data is not None:
            ...
            if len(jo.time.tolist()) > 10:
                first=jo.sort_values(by="time", ascending=False).ix[0]
                second=jo.sort_values(by="time", ascending=False).ix[1]
                last=jo.sort_values(by="time", ascending=False)[2:6]

                zong=jo.sort_values(by="time", ascending=False)[0:6]
                

                # if zong['open'].is_monotonic_increasing:
...
def get_laohu_analysis(n, url, li_code): 
    ...
    plt.savefig("fig_down_price_"+str(n)+".png")
    # plt.show()
