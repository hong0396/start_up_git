...
def get_grow_code(url, li_code): 
    ...
    for code_nm in li_code:
        ...
        if li_data is not None:
            ...
            if len(jo.time.tolist()) > 40:
                zong=jo.sort_values(by="time", ascending=False)[:40]
                # 第0个开盘价(涨)>1个收盘价(跌)
                # 1个开盘价(跌)>2个开盘价(涨)
                # 第2个开盘价(涨)>3个收盘价(跌)
                # 3个开盘价(跌)>4个开盘价(涨)
                # 4个开盘价(涨)>5个开盘价(涨)>6个开盘价(涨)>7个开盘价(涨)

                # 第i个开盘价(涨)>i+1个收盘价(跌)              和 AND      i+1个开盘价(跌)>i+2个开盘价(涨)         
                # 第i+2个开盘价(涨)>i+3个收盘价(跌)            和 AND      i+3个开盘价(跌)>i+4个开盘价(涨)
                # i+4个开盘价(涨)>i+5个开盘价(涨)>i+6个开盘价(涨)>i+7个开盘价(涨)
                for i in range(6):
                    if round(zong.iloc[i]['open'],2) >= round(zong.iloc[i+1]['close'],2) and round(zong.iloc[i+1]['open'],2) >= round(zong.iloc[i+2]['open'],2):
                        if round(zong.iloc[i+2]['open'],2) >= round(zong.iloc[i+3]['close'],2) and round(zong.iloc[i+3]['open'],2) >= round(zong.iloc[i+4]['open'],2):
                            
                            if round(zong.iloc[i]['open'],2) >= round(zong.iloc[i+2]['open'],2):
                                if round(zong.iloc[i+2]['open'],2) >= round(zong.iloc[i+4]['open'],2):
                                    # i+4个开盘价(涨)>i+5个开盘价(涨)>i+6个开盘价(涨)>i+7个开盘价(涨)
                                    if round(zong.iloc[i+4]['open'],2) >= round(zong.iloc[i+5]['open'],2) and round(zong.iloc[i+5]['open'],2) >= round(zong.iloc[i+6]['open'],2) and round(zong.iloc[i+6]['open'],2) >= round(zong.iloc[i+7]['open'],2):      
                                        
                                        if (zong.iloc[i]['close'] - zong.iloc[i]['open'])>= 0:
                                            if (zong.iloc[i+1]['close'] - zong.iloc[i+1]['open']) <= 0:
                                                if (zong.iloc[i+2]['close'] - zong.iloc[i+2]['open']) >= 0:
                                                    if (zong.iloc[i+3]['close'] - zong.iloc[i+3]['open']) <= 0:
                                                        if (zong.iloc[i+4]['close'] - zong.iloc[i+4]['open']) >= 0:
                                                            if (zong.iloc[i+5]['close'] - zong.iloc[i+5]['open']) >= 0:
                                                                if (zong.iloc[i+6]['close'] - zong.iloc[i+6]['open']) >= 0:
                                                                    if (zong.iloc[i+7]['close'] - zong.iloc[i+7]['open']) >= 0:
...
def get_laohu_analysis(n, url, li_code): 
    ...
    plt.savefig(date+"_fig_rollup_price_test_"+str(n)+".png")
    # plt.show()
...
write_li(date+'_up_code_test.txt',codee)
# codee=li_code[:1000]
