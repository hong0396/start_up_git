...
def get_grow_code(url, li_code): 
                ...
                for i in range(200):
                    if round(zong.iloc[i]['open'],2) >= round(zong.iloc[i+1]['close'],2) and round(zong.iloc[i+1]['open'],2) >= round(zong.iloc[i+2]['open'],2):
                        if round(zong.iloc[i]['open'],2) >= round(zong.iloc[i+2]['open'],2):
                            if round(zong.iloc[i+2]['open'],2) >= round(zong.iloc[i+3]['open'],2) and round(zong.iloc[i+3]['open'],2) >= round(zong.iloc[i+4]['open'],2) and round(zong.iloc[i+4]['open'],2) >= round(zong.iloc[i+5]['open'],2):      
    ...
    tmp_df=pd.DataFrame({'code': li_code_tmp ,  'starttime': li_num_tmp  })               
    return tmp_df
...
def get_laohu_analysis(n, url, pd_code): 
    fig, axes = plt.subplots(nrows=10, ncols=10, figsize=(30,30))
    li=[]
    nu_nu=0
    jo=pd.DataFrame()
    ...
    plt.savefig(date+"_fig_rollup_day_anaysis_"+str(n)+".png")
    # plt.show()
...
select.to_csv(date+'_up_code_time.csv')
# write_li(date+'_up_code.txt',codee)
# codee=li_code[:1000]
