    ...
    for code_nm in li_code:
        ...
        if li_data is not None:
            ...
            if len(jo.time.tolist()) > 250:
                ...
                # 第0个开盘价(跌)>1个收盘价(涨)
                # 1个开盘价(涨)>2个开盘价(跌)>3个开盘价(跌)>4个开盘价(跌)>5个开盘价(跌)
...
select.to_csv(date+'_down_code_date_radio_test.csv')
select_code.to_csv(date+'_down_code_time_radio_test.csv')
