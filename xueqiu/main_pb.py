...
#从Dataframe中只提取300股票的数据，就是特定行数据
a111=basic[basic['code'].isin(code)]


a111_new = a111.set_index('code') ##设置DataFrame索引
a111_new = a111_new.reindex(code) ##按照对DataFrame索引排序

print(code)
a111_new.to_csv("pb.csv")



pb=a111_new['pb'].values.tolist()
per=a111_new['per'].values.tolist()
trade=a111_new['trade'].values.tolist()
